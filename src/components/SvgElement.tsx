...
export function SvgElement({ element, containerRef }: SvgElementProps) {
  ...
  const preparedSvg = useMemo(() => prepareSvgForDisplay(element.svgContent), [element.svgContent]);
  ...
  return (
    <div
      ...
      dangerouslySetInnerHTML={{ __html: preparedSvg + '<div class="resize-handle"></div>' }}
    />
  );
}
